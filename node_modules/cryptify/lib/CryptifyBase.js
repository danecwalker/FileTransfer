...
class CryptifyBase {
  constructor(files, password, algorithm, encoding, silent, loose) {
    if (!_crypto.default) {
    ...
    if (algorithm && typeof algorithm === 'string') {
      ...
      this.algorithm = _const.DEFAULT.CIPHER_ALGORITHM;
    }
  ...
  isEncrypting() {
    if (!this.mode) {
      throw new Error("Command method not set. Expecting \"".concat(_types.CommandMode.Encrypt, "\" or ").concat(_types.CommandMode.Decrypt));
    }

    return this.mode === _types.CommandMode.Encrypt;
  }
  /**
  ...
  getIvFromStream(inputPath) {
    return _asyncToGenerator(function* () {
      return new Promise(resolve => {
        ...
        }).on(_types.StreamEvent.Data, persistedIv => iv = persistedIv).on(_types.StreamEvent.Close, () => resolve(iv));
      });
    })();
  }
  /**
  ...
  processStream(iStream, oStream) {
    ...
    return _asyncToGenerator(function* () {
      return new Promise((resolve, reject) => {
        iStream.pipe(_this3.cipher).on(_types.StreamEvent.Error, e => reject(e)).pipe(oStream).on(_types.StreamEvent.Close, () => resolve(null));
      });
    })();
  }

  getFilesSync() {
    return this.files.map(file => _fs.default.readFileSync(file, {
